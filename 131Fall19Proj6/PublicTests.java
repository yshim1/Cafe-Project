...
public class PublicTests {
	...
	public void testListSimpleRemove() {
		...
		//		for(int i = Food.FOOD_OBJECTS.length - 1; i >= 0; i--) {
		//			list.remove(Food.FOOD_OBJECTS[i]);
		//		
		//		}
		//		assertEquals(0, list.getSize());
		list.remove(MILK);
		assertEquals("[ Bacon, Cereal, Coffee, Croissant, Donut, Egg, Hashbrowns, Melon, Orange Juice, Pancakes, Pie, Toast, Waffle ]", list.toString());
	}
	...
	public void testAvailability() {
		...
		SortedListOfImmutables listToCheck = new SortedListOfImmutables(list);
		assertTrue(list.checkAvailability(listToCheck) == true);
		SortedListOfImmutables a = new SortedListOfImmutables(list);
		a.remove(BACON);
		assertTrue(a.checkAvailability(list) == false);
	}
