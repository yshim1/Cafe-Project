...
public class Restaurant {
	...
	public boolean addShipmentToInventory(SortedListOfImmutables list) {
		int wholeSaleCost = list.getWholesaleCost();
		if(wholeSaleCost > cash) {
			return false;
		}
		else {
			inventory.add(list);
			cash = cash - wholeSaleCost;
			return true;
		}
	}
	...
	public boolean placeOrder(Entree entree) {
		int retailValue = entree.getRetailValue();
		if(inventory.checkAvailability(entree.getFoodList()) == false) {
			return false;
		}
		else {
			cash = cash + retailValue;
